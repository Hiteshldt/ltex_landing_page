...
export default function StemCrossSection() {
  const layers = [
    {
      id: "outer",
      name: "Outer Sheath",
    ...
    >
      ...
      >
        ...
        <div style={{ fontWeight: 800, fontSize: 17, color: "var(--ink)", letterSpacing: "-0.01em" }}>
          {layers[active].name} <span style={{ color: "var(--ink-4)", fontWeight: 600 }}>→</span>{" "}
          <span style={{ color: layers[active].color }}>{layers[active].product}</span>
        </div>
